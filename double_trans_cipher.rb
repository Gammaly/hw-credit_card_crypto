module DoubleTranspositionCipher
  def self.encrypt(document, key)
    ...
    cipher_rows = blocks.each_with_index.map { |row, index| blocks[rows_key[index]] }

    # 4. sort columns of each row in predictibly random way
    cols_key = (0..width-1).to_a.shuffle(random: Random.new(key+1))
    rows_t = cipher_rows.transpose
    cipher_cols = rows_t.each_with_index.map { |row, index| rows_t[cols_key[index]] }.transpose
  ...
  def self.decrypt(ciphertext, key)
    ...
    cipher_cols = blocks_t.each_with_index.map { |row, index| blocks_t[cols_key.index(index)] }.transpose

    rows_key = (0..width-1).to_a.shuffle(random: Random.new(key))
    cipher_rows = cipher_cols.each_with_index.map { |row, index| cipher_cols[rows_key.index(index)] }
